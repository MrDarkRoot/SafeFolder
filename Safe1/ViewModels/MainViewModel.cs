...
namespace Safe1.ViewModels
{
    public class MainViewModel : BaseViewModel
    {
        ...
        private async Task ExecuteHandleAction(object parameter)
        {
            ...
            // default action if called via command
            await HandleFolderActionWithTypeAsync("Action", model).ConfigureAwait(false);
        }

        public async Task HandleFolderActionWithTypeAsync(string action, ProtectedFolderModel model)
        {
            ...
            if (action == "Lock")
            {
                ...
                else if (model.ProtectionMode == ProtectionMode.LOCKED)
                {
                    ...
                    try
                    {
                        var restored = await QuickProtectService.QuickUnlockAsync(model.FolderPath).ConfigureAwait(false);
                        ConfigurationManager.UpdateFolderPath(_db, model.FolderPath, restored);
                        ConfigurationManager.UpdateProtectionMode(_db, restored, "NORMAL");
                        model.FolderPath = restored;
            ...
            {
                // quick access: create secure workspace and open
                if (model.ProtectionMode == ProtectionMode.LOCKED)
                {
                    try
                    {
                        var stw = SecureAccessService.CreateSecureWorkspace();
                        // placeholder: open folder or stream files into stw
                    }
                    catch { }
                }
            }
            ...
            {
                if (model.ProtectionMode == ProtectionMode.ENCRYPTED)
                {
                    try
                    {
                        string master = null;
                        ...
                        var progress = new Progress<double>(p => { /* TODO: update UI */ });
